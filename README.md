...
> **注意**: Azure Developer CLI (azd) には環境ごとに`.env`ファイルを管理する機能があります。当サンプルではazdが管理する`.env`ファイルがAzureへのデプロイだけでなく、ローカル実行やビルド、環境のクリーンアップ時に参照されます。ユーザーが直接`.env`ファイルを操作する必要はありません。
